<x-app-layout>
    <x-slot name="header">
        ...
        <a href="{{ route('audit_hukum.create') }}" class="btn btn-primary">Add Audit Hukum</a>
    </x-slot>

    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                @if ($auditHukum->where('user_id', auth()->user()->id)->isEmpty())
                    <p>No audit hukum found.</p>
                @else
                    @foreach ($auditHukum as $p)
                        @if ($p->user_id == auth()->user()->id)
                            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                                ...
                                <p>Unggah File:
                                    <a href="{{ asset('files/audithukum/' . $p->unggah_file) }}" target="_blank"
                                        class="link-secondary link-offset-2 link-underline-opacity-25 link-underline-opacity-100-hover">Lihat
                                        File</a>
                                </p>
                                <div class="mt-4">
                                    <a href="{{ route('audit_hukum.edit', $p) }}" class="btn btn-primary">Edit</a>
                                    <form action="{{ route('audit_hukum.destroy', $p) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <x-jet-danger-button type="submit">Hapus</x-jet-danger-button>
                                    </form>
                                    <a href="{{ asset('files/audithukum/' . $p->unggah_file) }}"
